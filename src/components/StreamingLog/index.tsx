...
const getCaterories = (core_ids: { [key: string]: boolean }) => {
  const cores = Object.keys(core_ids).length
  const tasks = cores === 1 ? ['Tasks'] : Object.keys(core_ids).map((i) => `Tasks-E${i}`)
...
const StreamingLog = (props: { dataUrl?: string; data?: string; cores?: number; title?: string; desc?: string }) => {
  ...
  const getData = useCallback(async () => {
    ...
    // setCores(cores)
    const cats = getCaterories(executorIDs)
    setCategories(cats)
    // console.log(cores)
    ...
    const generateData = (obj: any, name: string, index: number) => {
      Object.keys(obj).forEach((key: any) => {
        ...
        let idx = index
        if (executorId) {
          const i = cats.find((cat) => cat === `Tasks-E${executorId}`)
          idx = (i || 0) as number
        }
